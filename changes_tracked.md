...
### Commit 15 — Synthetic data in every band: tables from a fake spectrum under HEASoft

- HEASoft lives in the `henv` conda environment (`$CONDA_PREFIX/heasoft`,
  initialised by the activation hook), so the data notebook's XSPEC route is the
  one that runs: `cloak.synthetic.spectrum` fakes a `tbabs*powerlaw` spectrum
  (`nH` 0.3e22, Γ 1.8, norm 1e-3, 100 ks through the IC 10 X-1 ACIS response,
  which stays untracked) and `cloak.flux_table` tabulates one table per band in
  `BANDS` (600 columns over 1e19–1e26 cm⁻²) into `synthetic_data/tables/`. Both
  scripts ran against a live HEASoft for the first time: `flux_table.py`
  crashed on a spectrum without background (PyXspec raises on
  `spectrum.background` instead of returning None) and is fixed. The table
  fits use cstat: the chi-square fit of the low-count fake spectrum was biased
  (Γ 1.88, `nH` 0.32 for inputs 1.8, 0.30); cstat recovers 1.793 and 0.299.
- Light curves of both systems in all four bands, generated by the unchanged
  `cloak.synthetic.lightcurve`. A system's `target_rate` is its broad-band
  rate; a sub-band's rate follows from the fake spectrum's flux-per-count-rate
  factors (`spec/band_factors.json`, the spectrum through the response)
  rescaled so that the broad band meets its target (System A: 72.6k broad,
  32.7k soft, 22.8k medium, 24.9k hard counts; without factors a band would get
  its photon-flux share). A light curve is regenerated when its table or the
  fiducial definition changed, not only when the file is missing.
- `figlib.floor_prior()` replaces the fixed numbers of `FLOOR_PRIOR`: 3 % of
  System A's out-of-eclipse broad flux with a 2 % width, cut at 10 %, so the
  calibration batch's floor prior follows the flux scale of the table in use
  (the fake spectrum is 3 times brighter than the example table).
- `.gitignore` ignores `synthetic_data/spec/` except `band_factors.json`. The
  tracked example table `flux_vs_nH_tbabs_broad.csv` stays for the tests and
  examples; the notebooks and the figure code use `tables/` exclusively, so
  the four paper fits were redone on the regenerated broad light curve.
- fig06 (energy dependence of the eclipse) and fig14 (cross-band prediction
  from the broad-band posterior) now have their inputs and are produced; fig06
  gets plain keV ticks, fig14 short panel titles (the long ones collided with
  the panel letters).
- Verified: tests 14/14; both notebooks execute at full size (the data
  notebook under HEASoft); the four fits redone on the new broad light curve
  (24.6 + 15.7 + 15.5 + 15.7 min; still flagged unconverged, τ_max 168–283
  steps). Half-depth eclipse widths: System A soft 0.333, medium 0.264, broad
  0.105, hard 0.069; System B 0.310 / 0.230 / 0.104 / 0.083. The broad-band
  posterior predicts the soft / medium / hard synthetic light curves with
  χ²_eff/n = 1.04 / 0.94 / 1.09 (292 / 211 / 231 bins).
