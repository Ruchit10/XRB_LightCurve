...
def load_flux_vs_nh_csv(csv_path: str) -> pd.DataFrame:
    """
    Load flux vs nH CSV file generated by compute_flux_vs_nH.py.
    
    Args:
        csv_path: Path to CSV file with columns nH_cm2, nH_1e22, flux_soft_ph, flux_hard_ph
        
    Returns:
        DataFrame with flux vs nH data
    ...
    required_cols = ["nH_1e22", "flux_soft_ph", "flux_hard_ph"]
    missing = [col for col in required_cols if col not in df.columns]
    if missing:
        raise ValueError(f"CSV missing required columns: {missing}")
    
    # Remove any NaN or invalid values
    df = df[df["nH_1e22"].notna() & df["flux_soft_ph"].notna() & df["flux_hard_ph"].notna()]
    df = df[(df["flux_soft_ph"] > 0) & (df["flux_hard_ph"] > 0)]
...
def interpolate_flux_from_nh(
    ...
    if band == "soft":
        ...
        flux_col = "flux_hard_ph"
    else:
        raise ValueError(f"Band must be 'soft' or 'hard', got: {band}")
    ...
    if np.any(nh_1e22 < nh_min) or np.any(nh_1e22 > nh_max):
        warnings.warn(
            f"Some nH values are outside CSV range [{nh_min:.3f}, {nh_max:.3f}] 1e22 cm^-2. "
...
def simulate_lightcurve(
    ...
    if flux_method == "legacy":
        ...
        df_flux = load_flux_vs_nh_csv(flux_csv_path)
        nfl_hard_av = interpolate_flux_from_nh(fl, df_flux, "hard")
        nfl_hard_cv = interpolate_flux_from_nh(fl2, df_flux, "hard")
        nfl_soft_av = interpolate_flux_from_nh(fl, df_flux, "soft")
        nfl_soft_cv = interpolate_flux_from_nh(fl2, df_flux, "soft")
        ...
        df_flux = load_flux_vs_nh_csv(flux_csv_path)
        A_hard, B_hard = fit_exponential_to_csv(df_flux, "hard")
        A_soft, B_soft = fit_exponential_to_csv(df_flux, "soft")
        
        nfl_hard_av = A_hard * np.exp(-B_hard * fl)
        nfl_hard_cv = A_hard * np.exp(-B_hard * fl2)
        nfl_soft_av = A_soft * np.exp(-B_soft * fl)
        nfl_soft_cv = A_soft * np.exp(-B_soft * fl2)
    ...
    # Only add photon count columns in legacy mode
    if flux_method == "legacy":
...
def main():
    ...
    parser.add_argument(
        ...
        "Default: auto-scale to mean(fl)=0.589537",
    )
    parser.add_argument(
        "--lam2",
        type=float,
        ...
        "Default: auto-scale to mean(fl2)=0.589537",
    )
    parser.add_argument(
        "--output",
        type=str,
